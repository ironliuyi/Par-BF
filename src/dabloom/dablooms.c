 ...
 */
void hash_func(counting_bloom_t *bloom, KEY_TYPE key, uint32_t *hashes)
{
    int i;
    if(sizeof(key)<2*sizeof(uint32_t))
    {
    	DEBUG_PRINT("%s\n","Key length is not enough");
    	abort();
    }
    uint32_t bits_mask = 0xffffffff;
    uint32_t h1 = key & bits_mask;
    uint32_t h2 = (key>>32) & bits_mask;
...
counting_bloom_t *counting_bloom_init(size_t bloom_size, double error_rate, long offset)
{
    ...
    bloom->bitmap = NULL;
    bloom->capacity = (unsigned int) ceil(bloom_size*pow(log(2), 2)/fabs(log(error_rate)));
    DEBUG_PRINT("the capacity is %d\n",bloom->capacity);
    bloom->error_rate = error_rate;
    bloom->offset = offset + sizeof(counting_bloom_header_t);
    bloom->nfuncs = (unsigned int) ceil(log(1 / error_rate) / log(2));
    bloom->counts_per_func = (unsigned int) ceil( bloom_size / bloom->nfuncs );
...
counting_bloom_t *new_counting_bloom(size_t bloom_size, double error_rate, const char *filename)
{
    ...
    cur_bloom = counting_bloom_init(bloom_size, error_rate, 0);
...
counting_bloom_t *new_counting_bloom_from_scale(scaling_bloom_t *bloom)
{
    ...
    error_rate = bloom->error_rate * (pow(ERROR_TIGHTENING_RATIO, bloom->num_blooms));
    bloom_size = bloom->scaling_bloom_size * (pow (SCALABLE_GROWTH_RATIO,bloom->num_blooms));

    if ((bloom->blooms = realloc(bloom->blooms, (bloom->num_blooms + 1) * sizeof(counting_bloom_t *))) == NULL) {
        fprintf(stderr, "Error, could not realloc a new bloom filter\n");
        return NULL;
    }
    cur_bloom = counting_bloom_init(bloom_size, error_rate, bloom->num_bytes);
...
counting_bloom_t *new_counting_bloom_from_file(size_t bloom_size, double error_rate, const char *filename)
{
    ...
    bloom = counting_bloom_init(bloom_size, error_rate, 0);
...
scaling_bloom_t *scaling_bloom_init(size_t bloom_size, double error_rate, const char *filename, int fd)
{
    ...
    bloom->header = (scaling_bloom_header_t *) bloom->bitmap->array;
    bloom->scaling_bloom_size = bloom_size;
    bloom->error_rate = error_rate;
    bloom->num_blooms = 0;
...
scaling_bloom_t *new_scaling_bloom(size_t bloom_size, double error_rate, const char *filename)
{
    ...
    bloom = scaling_bloom_init(bloom_size, error_rate, filename, fd);
